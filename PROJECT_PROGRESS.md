...
| **階段 3** | 個人月曆與每週預設設定頁面 (UI/UX) | ✅ 已完成 | `PersonalController`、`Calendar.cshtml` (CSS Grid + Modal AJAX 異動)、`WeeklySetup.cshtml`、**全站深色模式切換 (Dark Mode)**、**首頁真實資料庫端點接合 (HomeDashboardViewModel)、精簡重複按鈕、與產出系統設計文件 (design_document.md)**。 |
